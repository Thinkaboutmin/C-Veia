#ifndef AI_MIN_MAX_PRUNED_H
#define AI_MIN_MAX_PRUNED_H
...
namespace tic_tac_toe {
    template<typename T>
    class AIMinMaxPruned : public Player<T> {
    private:
        std::vector<int> tableValues;
        TicTacToe<T> * gameJudge = nullptr;

    public:
        AIMinMaxPruned(T symbol) : Player<T>(symbol) {}
        ...
        PlayerMove<T> getMove(PlainTable<T> &table) override {
            ...
            // Fix game flow
            Player<T> * player = nullptr;
            do {
                player = &this->gameJudge->getNextPlayer();
            } while (player != this);
    ...
    private:
        std::pair<PlayerMove<T>, short> minMaxMoves(PlainTable<T> &table, int depth, int playerIndex, short alpha = std::numeric_limits<short>::min(), 
            ...
            for(unsigned short i = 1; i <= cells.size(); ++i) {
                ...
                for (const unsigned short & j : cells[i - 1]) {
                    ...
                    auto newMove = this->minMaxMoves(table, depth - 1, this->getNextPlayer(playerIndex), alpha, beta);
                    newMove.first = PlayerMove<T>(i, j, player->getPlayerSymbol());
                    table.setCellValue(i, j, &table.getEmptyValue());
                    
                    // Max
                    if (player == this) {      
                        if (newMove.second > alpha) {
